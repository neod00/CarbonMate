...
export function SensitivityAnalysisStep() {
    ...
    const baselineCFP = useMemo(() => {
        let total = 0
        const electricityFactor = getDefaultElectricityFactor()
        const transportFactor = getDefaultTransportFactor()

        // 원자재
        if (activityData.raw_material_weight) {
            total += activityData.raw_material_weight * 2.0  // 기본 배출계수
        }

        // 전력
        if (activityData.electricity) {
            total += activityData.electricity * electricityFactor.value
        }

        // 가스
        if (activityData.gas) {
            total += activityData.gas * 2.23
        }

        // 운송
        if (activityData.transport_weight && activityData.transport_distance) {
            const tkm = (activityData.transport_weight * activityData.transport_distance) / 1000
            total += tkm * transportFactor.value
        }

        // 사용 단계
        if (activityData.use_electricity && activityData.use_years) {
            total += activityData.use_electricity * activityData.use_years * electricityFactor.value
        }

        // 폐기
        if (activityData.waste_weight) {
            const recyclingRate = activityData.recycling_rate || 0
            total += activityData.waste_weight * (1 - recyclingRate) * 0.58
            total += activityData.waste_weight * recyclingRate * (-0.8)
        }
    ...
    const calculateEmission = (data: typeof activityData): number => {
        ...
        if (data.raw_material_weight) {
            total += data.raw_material_weight * 2.0
        }
        if (data.electricity) {
            total += data.electricity * electricityFactor.value
        }
        if (data.gas) {
            total += data.gas * 2.23
        }
        if (data.transport_weight && data.transport_distance) {
            const tkm = (data.transport_weight * data.transport_distance) / 1000
            total += tkm * transportFactor.value
        }
        if (data.use_electricity && data.use_years) {
            total += data.use_electricity * data.use_years * electricityFactor.value
        }
        if (data.waste_weight) {
            const recyclingRate = data.recycling_rate || 0
            total += data.waste_weight * (1 - recyclingRate) * 0.58
            total += data.waste_weight * recyclingRate * (-0.8)
        }
    ...
    const runAnalysis = () => {
        ...
        setTimeout(() => {
            const result = performComprehensiveSensitivityAnalysis({
                baselineCFP,
                activityData,
                electricityGridId: activityData.electricity_grid || 'electricity_korea_grid_2023',
                            ...
                            <div className="w-1/2 flex justify-end">
                                <div 
                                    className={`h-6 rounded-l ${
                                        isSignificant ? 'bg-red-500' : 'bg-red-500/50'
                                    }`}
                            ...
                            <div className="w-1/2 flex justify-start">
                                <div 
                                    className={`h-6 rounded-r ${
                                        isSignificant ? 'bg-green-500' : 'bg-green-500/50'
                                    }`}
                            ...
                            <tbody>
                                {typeScenarios.slice(0, 5).map((scenario) => (
                                    <tr 
                                        key={scenario.id} 
                                        className={`border-b ${
                                            scenario.isSignificant ? 'bg-yellow-500/5' : ''
                                        }`}
                                    >
                                        ...
                                        <td className="py-2 px-3 text-right">
                                            <span className={`font-medium ${
                                                scenario.percentageChange >= 0 ? 'text-red-500' : 'text-green-500'
                                            }`}>
